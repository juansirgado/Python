...
# set path=%path%;C:\Users\Pykaxoo\AppData\Local\Packages\PythonSoftwareFoundation.Python.3.12_qbz5n2kfra8p0\LocalCache\local-packages\Python312\Scripts
...
def check_user():
    CryptoText.int_parts = 0 
    for int_textpart in range(1, 999, 1):
        if(os.path.isfile("local-storage/CryptoText_" + "{:03d}".format(int_textpart) + CryptoText.str_sufix)):
            CryptoText.int_parts += 1
    if(CryptoText.int_parts > 0):
...
def cmd_update():
    ...
    if(CryptoText.bol_new_user != True):
       if(CryptoText.bol_user != True):
           mb.showerror(title="Error", message="User not logged!")
           return()
    for int_textpart in range(1, CryptoText.int_parts + 1, 1):
        os.remove("local-storage/CryptoText_" + "{:03d}".format(int_textpart) + CryptoText.str_sufix)
    file_write()
    if(CryptoText.bol_new_user != True):
...
def cmd_delete():
    ...
    os.remove("local-storage/PublicKey" + CryptoText.str_sufix)
    os.remove("local-storage/PrivateKey" + CryptoText.str_sufix)
    os.remove("local-storage/Signature" + CryptoText.str_sufix)
    for int_textpart in range(1, CryptoText.int_parts + 1, 1):
        os.remove("local-storage/CryptoText_" + "{:03d}".format(int_textpart) + CryptoText.str_sufix)
    ent_user.delete(0, tk.END)
    ent_pass.delete(0, tk.END)
...
def file_read():
    ...
    for int_textpart in range(1, CryptoText.int_parts + 1, 1):
        with open("local-storage/CryptoText_" + "{:03d}".format(int_textpart) + CryptoText.str_sufix, mode="rb") as input_file:
...
def file_write():
    ...
    CryptoText.str_text = txt_text.get("1.0", tk.END)
    CryptoText.int_parts = 0
    int_textlen = len(CryptoText.str_text)
    for int_textcut in range(0, int_textlen, 101):
        CryptoText.int_parts += 1
        str_textpart = CryptoText.str_text[int_textcut:int_textcut + 101] 
        if(CryptoText.int_parts == 1):
            byt_signature = rsa_sign(str_textpart, privateKey)
        byt_encrypt = rsa_encrypt(str_textpart, publicKey)
        with open("local-storage/CryptoText_" + "{:03d}".format(CryptoText.int_parts) + CryptoText.str_sufix, mode="wb") as output_file:
...
#===========================================================
    cls_kdf = PBKDF2HMAC(
        ...
        iterations=1048576)
#===========================================================
# Fernet - String encode Base64 to Fernet Key
#===========================================================
...
def rsa_generate_keys():
    ...
    with open("local-storage/PublicKey" + CryptoText.str_sufix, "wb") as buf_writer:
        byt_encrytp = fernet_encrypt(publicKey.save_pkcs1("PEM"))
        buf_writer.write(byt_encrytp)
    with open("local-storage/PrivateKey" + CryptoText.str_sufix, "wb") as buf_writer:
        byt_encrytp = fernet_encrypt(privateKey.save_pkcs1("PEM"))
        buf_writer.write(byt_encrytp)
    return()

def rsa_recover_keys():
    with open("local-storage/PublicKey" + CryptoText.str_sufix, "rb") as buf_reader:
        byt_decrypt = fernet_decrypt(buf_reader.read())
        publicKey = rsa.PublicKey.load_pkcs1(byt_decrypt)
    with open("local-storage/PrivateKey" + CryptoText.str_sufix, "rb") as buf_reader:
        byt_decrypt = fernet_decrypt(buf_reader.read())
...
def rsa_sign(str_decode:str, privateKey):
    ...
    with open("local-storage/Signature" + CryptoText.str_sufix, "wb") as buf_writer:
        buf_writer.write(byt_encrypt) 
    return(byt_signature)

def rsa_verify(str_decode:str, publicKey):
    with open("local-storage/Signature" + CryptoText.str_sufix, "rb") as buf_reader:
...
win_main.resizable(width=True, height=True)
win_main.geometry("600x800")
...
lbl_login = tk.Label(master=frm_text ,text="", bg="white")
lbl_login.grid(row=0, column=1, padx=5, pady=5, sticky="nsew")

txt_text = st.ScrolledText(master=frm_text, width=1024, height=1024, undo=True)
txt_text.grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky="nsew")

#===========================================================
# GUI - Control Frame
#===========================================================
frm_ctrl = tk.Frame(master=win_main, width=10, height=10, borderwidth=5)
frm_ctrl.pack(fill=tk.BOTH,  side=tk.BOTTOM, expand=True)
