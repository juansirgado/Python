...
def cmd_update():
    ...
    if(CryptoText.bol_new_user != True):
       if(CryptoText.bol_user != True):
           mb.showerror(title="Error", message="User not logged!")
           return()    
    file_write("local-storage/CryptoText" + CryptoText.str_sufix)
    if(CryptoText.bol_new_user != True):
...
def cmd_delete():
    ...
    for int_textpart in (1, CryptoText.int_parts, 1):
...
def file_write():
    ...
    CryptoText.str_text = txt_text.get("1.0", tk.END)
    int_textpart = 0
    for int_textcut in (0, len(CryptoText.str_text), 117):
        int_textpart += 1
        ...
        with open("local-storage/CryptoText_" + "{:03d}".format(int_textpart) + CryptoText.str_sufix, mode="wb") as output_file:
