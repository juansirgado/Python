...
# Date         : 2025-06-04                                #
# Version      : 1.0                                       #
# Description  : Load Drive Info in the PostgreSQL         #
#----------------------------------------------------------#
...
#----------------------------------------------------------#
def SM_Drive(str_drive=""):
    #----------------------------------------------------------#
    log.system_log("SM_Drive()", "Start")
    #----------------------------------------------------------#
    sql_list = [] # empty list
    #----------------------------------------------------------#
    ...
    sql_prefix += "'Drive',"
    sql_prefix += "'" + str_drive + "',"
    #----------------------------------------------------------#
    # Create the Drive Read in MB SQL
    #----------------------------------------------------------#
    sql_insert  = sql_prefix
    sql_insert += str(psu.disk_io_counters(perdisk=True)[str_drive].read_bytes / 2 ** 20) + ","
    sql_insert += "'MB',"
    sql_insert += "'Total read',"
    sql_insert += "'Read');"
    sql_list.append(sql_insert)
    #----------------------------------------------------------#
    # Create the Drive Write in MB SQL
    #----------------------------------------------------------#
    sql_insert  = sql_prefix
    sql_insert += str(psu.disk_io_counters(perdisk=True)[str_drive].write_bytes / 2 ** 20) + ","
    sql_insert += "'MB',"
    sql_insert += "'Total write',"
    sql_insert += "'Write');"
    sql_list.append(sql_insert)
    #----------------------------------------------------------#
    log.system_log("SM_Drive()", "Stop")
    #----------------------------------------------------------#
    return(sql_list)
#----------------------------------------------------------#
# That is all Folks!
